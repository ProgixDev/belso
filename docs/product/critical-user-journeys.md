...
| CUJ-06 | Publish a listing            | Sign in to the back-office → create a listing → write it in French → publish it → see it live on the French site, and on the English site with the untranslated note → add the English later → rename it and find the old address still works → take it off the site | `e2e/admin-auth.spec.ts`, `e2e/listing-editor.spec.ts` | `5*-*`, `6*-*` |
