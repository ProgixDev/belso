...
SEC-RATE-003   | P2 | Limiters read the **last** X-Forwarded-For hop, never the first. The first entry is whatever the client sent; the last is what the nearest proxy observed, so the limiter is correct whether or not the proxy sanitises. Traefik 3.7 with no forwardedHeaders.trustedIPs does sanitise — but the limiter must not depend on a setting in a file this repository does not own | network.test.ts | lib/network.ts clientAddress()
```
