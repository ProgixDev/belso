...
SEC-RATE-003   | P2 | Limiters read the **last** X-Forwarded-For hop, never the first. The first entry is whatever the client sent; the last is what the nearest proxy observed, so the limiter is correct whether or not the proxy sanitises. **Measured 01/09** against the live Traefik: a client-supplied X-Forwarded-For is discarded and replaced with the real address, so the proxy does sanitise — but the limiter must not depend on a setting in a file this repository does not own | network.test.ts; spike recorded in specs/013-belso-deploy/tasks.md T-06 | lib/network.ts clientAddress()
```
