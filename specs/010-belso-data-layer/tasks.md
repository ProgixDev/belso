...
- [ ] **T0** Confirm spec 009 Phase 5 is filed (see plan _Overlap check_); branch `feat/010-data-layer` · done: `docs/reports/009-belso-map.md` exists, branch created
- [ ] **T1** Postgres on the VPS: `/docker/belso-db/docker-compose.yml`, `postgres:17`, **no `ports:` entry**, own network, volume `belso_db_data`. App reaches it over the Docker network; developers reach it over an SSH tunnel · done: `psql` works through the tunnel, and `nmap` from outside shows 5432 closed
- [ ] **T2** Golden snapshot: `src/features/properties/repository.golden.test.ts` serializes all six read functions' output from today's fixtures into a committed JSON file · done: test green against fixtures, snapshot committed (this is the AC-1 oracle — it must be written **before** any SQL)

## Phase 1 — schema and seed (AC-2, AC-3, AC-8)

- [ ] **T3** `db/migrations/0001_districts_properties.sql` — `districts`, `properties`, `property_translations`, `property_media`. `publication` enum (`draft`/`published`/`archived`) **separate from the existing `status`** · done: applies cleanly to an empty database
- [ ] **T4** [P] `db/migrations/0002_slug_history.sql` — `property_slug_history` (slug, locale, property, retired_at) · done: applies cleanly
- [ ] **T5** [P] `db/migrations/0003_enquiries.sql` — `enquiries` + the rate-limit counter table · done: applies cleanly
- [ ] **T6** `scripts/migrate.mjs` — applies pending numbered files in order, records them in `schema_migrations`, refuses to reapply · done: running twice applies nothing the second time
- [ ] **T7** `scripts/seed.mjs` — today's twenty fixtures into the database, **idempotent** (upsert on `reference`) · done: run twice → 20 properties, not 40 (**AC-8**)
- [ ] **T8** `src/core/db.ts` — `server-only` pool from `DATABASE_URL`; add the var to `src/core/env.ts` and `.env.example`, using the blank-is-unset `configured()` guard · done: `pnpm typecheck` green, `pnpm secrets:check` green
...
- [ ] AC-5 → T17 · [ ] AC-6 → T20 · [ ] AC-7 → T12 · [ ] AC-8 → T7
